...
def main():
    ...
    for i in range(0, img_count - 1):
        for j in range(i + 1, img_count):
            ...
            pts1 = np.hstack((x[idx, i].reshape((-1, 1)),
                             y[idx, i].reshape((-1, 1))))
            pts2 = np.hstack((x[idx, j].reshape((-1, 1)),
                             y[idx, j].reshape((-1, 1))))
            idx = np.array(idx).reshape(-1)
            # show_matches(imgs[i], imgs[j], pts1, pts2, (0,255,255), None)
            if len(idx) > 8:
                F_best, chosen_idx = ransac(pts1, pts2, idx)
                print('At image : ', i, j, '|| Number of inliers: ',
                      len(chosen_idx), '/', len(idx))

                # Store the best fundamental matrix
                f_mat[i, j] = F_best
                ...
                pts1 = np.hstack(
                    (x[idx, i].reshape((-1, 1)), y[idx, i].reshape((-1, 1))))
                pts2 = np.hstack(
                    (x[idx, j].reshape((-1, 1)), y[idx, j].reshape((-1, 1))))
    ...
    X_refined = NonLinearTriangulation(
        K, pts1, pts2, X, R1_, C1_, R_chosen, C_chosen)
    X_refined = X_refined / X_refined[:, 3].reshape(-1, 1)

    mean_error1 = meanReprojectionError(
        X, pts1, pts2, R1_, C1_, R_chosen, C_chosen, K)
    mean_error2 = meanReprojectionError(
        X_refined, pts1, pts2, R1_, C1_, R_chosen, C_chosen, K)
...
# Registering other cameras
    ...
    for i in range(2, img_count):
        ...
        pts_i = np.hstack((x[feature_idx_i, i].reshape(-1, 1),
                          y[feature_idx_i, i].reshape(-1, 1)))
        X = X_all[feature_idx_i, :].reshape(-1, 3)

        R_init, C_init = PnPRANSAC(
            K, pts_i, X, n_iterations=1000, error_thresh=15)
        ...
        print("Error linear PnP: ", errorLinearPnP,
              " Error non linear PnP: ", errorNonLinearPnP)
        ...
        for j in range(0, i):
            ...
            x1 = np.hstack((x[idx_X_pts, j].reshape((-1, 1)),
                           y[idx_X_pts, j].reshape((-1, 1))))
            x2 = np.hstack((x[idx_X_pts, i].reshape((-1, 1)),
                           y[idx_X_pts, i].reshape((-1, 1))))

            X = LinearTriangulation(K, C_set_[j], R_set_[j], Ci, Ri, x1, x2)
            X = X/X[:, 3].reshape(-1, 1)

            LT_error = meanReprojectionError(
                X, x1, x2, R_set_[j], C_set_[j], Ri, Ci, K)

            X = NonLinearTriangulation(
                K, x1, x2, X, R_set_[j], C_set_[j], Ri, Ci)
            X = X/X[:, 3].reshape(-1, 1)

            nLT_error = meanReprojectionError(
                X, x1, x2, R_set_[j], C_set_[j], Ri, Ci, K)
        ...
        if BA:
            ...
            for k in range(0, i+1):
                ...
                # print(idx_X_pts)
                new_x = np.hstack((x[idx_X_pts, k].reshape(
                    (-1, 1)), y[idx_X_pts, k].reshape((-1, 1))))
                X = X_all[idx_X_pts]
                BA_error = reprojectionErrorPnP(
                    X, new_x, K, R_set_[k], C_set_[k])
