...
def getObservationsIndexAndVizMat(X_found, filtered_feature_flag, nCam):
    ...
    for n in range(nCam + 1):
        visiblity_matrix = np.hstack(
            (visiblity_matrix, filtered_feature_flag[X_index, n].reshape(-1, 1)))
    ...
    for i in range(h):
        for j in range(w):
            if visiblity_matrix[i, j] == 1:
                pt = np.hstack(
                    (visible_feature_x[i, j], visible_feature_y[i, j]))
                pts2D.append(pt)
...
def bundle_adjustment_sparsity(X_found, filtered_feature_flag, nCam):
    ...
    X_index, visiblity_matrix = getObservationsIndexAndVizMat(
        X_found.reshape(-1), filtered_feature_flag, nCam)
...
def project(points_3d, camera_params, K):
    def projectPoint_(R, C, pt3D, K):
        P2 = np.dot(K, np.dot(R, np.hstack(
            (np.identity(3), -C.reshape(3, 1)))))
...
def BundleAdjustment(X_all, X_found, feature_x, feature_y, filtered_feature_flag, R_set_, C_set_, K, nCam):
    ...
    x1 = res.x
    number_of_cam = nCam + 1
    optimized_camera_params = x1[:number_of_cam *
                                 6].reshape((number_of_cam, 6))
